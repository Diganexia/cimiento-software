...
export default function Arqueos() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-6">Historial de arqueos</h1>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {data.map((a) => (
              <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">{a.caja}</td>
                <td className="px-4 py-3 text-gray-600 dark:text-gray-300 text-xs">{new Date(a.abierto_at).toLocaleString('es-AR')}</td>
                <td className="px-4 py-3 text-gray-600 dark:text-gray-300 text-xs">{a.cerrado_at ? new Date(a.cerrado_at).toLocaleString('es-AR') : '—'}</td>
                <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{a.usuario_apertura}</td>
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${a.estado === 'abierto' ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300'}`}>
                    {a.estado}
                  </span>
                </td>
                <td className="px-4 py-3 text-right space-x-2">
                  <button onClick={() => navigate(`/tesoreria/arqueos/${a.id}`)} className="text-blue-600 hover:text-blue-800 text-xs">Ver</button>
                  <button onClick={() => downloadPdfArqueo(a.id).catch(console.error)} className="text-gray-500 dark:text-gray-400 hover:text-gray-700 text-xs">PDF</button>
                </td>
              </tr>
            ))}
            {!data.length && (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400 dark:text-gray-500">Sin arqueos</td></tr>
            )}
          </tbody>
        </table>
      </div>
