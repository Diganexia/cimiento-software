    ...
    <div className="p-6 max-w-5xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="text-right">
          ...
          <p className="text-2xl font-bold text-gray-800">${fmt(totalVentas)}</p>
          <p className="text-xs text-gray-400">{total} comprobantes</p>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {ventas.map((v) => (
              <tr key={v.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-700">{TIPO_LABEL[v.tipo_comprobante] || v.tipo_comprobante}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${ESTADO_BADGE[v.estado]}`}>
                    {v.estado}
                  </span>
                </td>
                <td className="px-4 py-3 text-gray-600 text-xs capitalize">{v.tipo_pago}</td>
                <td className="px-4 py-3 text-right font-semibold text-gray-800">${fmt(v.total)}</td>
                <td className="px-4 py-3 text-right">
                  ...
                  <button onClick={() => navigate(`/ventas/${v.id}`)}
                    className="text-gray-500 hover:text-gray-700 text-xs">Ver</button>
                </td>
              </tr>
            ))}
            {!ventas.length && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">Sin ventas para este cliente</td></tr>
            )}
          </tbody>
        </table>
      </div>
