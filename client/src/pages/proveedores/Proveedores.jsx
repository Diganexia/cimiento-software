  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-xl font-semibold text-gray-800 dark:text-gray-100">Proveedores</h1>
        {canCreate && (
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-700">
            <tr>
              ...
              <th className="text-left px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">CUIT</th>
              <th className="text-left px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">Teléfono</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {loading ? (
              <tr><td colSpan={6} className="text-center py-8 text-gray-400 dark:text-gray-500">Cargando...</td></tr>
            ) : proveedores.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-8 text-gray-400 dark:text-gray-500">Sin proveedores</td></tr>
            ) : (
              proveedores.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="px-4 py-3">
                    ...
                    {p.razon_social && p.razon_social !== p.nombre && (
                      <p className="text-xs text-gray-400 dark:text-gray-500">{p.razon_social}</p>
                    )}
                  </td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300 font-mono text-xs">{p.cuit || '—'}</td>
