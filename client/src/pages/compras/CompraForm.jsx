  ...
  return (
    <div className="p-6 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-semibold text-gray-800 dark:text-gray-100">{isEdit ? 'Editar compra' : 'Nueva compra'}</h1>
      </div>

      <div className="space-y-4">
        {/* Cabecera */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5">
          <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-4">Datos de la compra</h2>
                ...
                {resultados.map((p) => (
                  <li key={p.id}>
                    <button type="button" onClick={() => agregarItem(p)}
                      ...
                      <span className="text-xs text-gray-500 dark:text-gray-400">Costo: ${parseFloat(p.precio_costo).toLocaleString('es-AR')}</span>
                    </button>
                  </li>
                ))}
              </ul>
          ...
          {items.length > 0 ? (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-700">
                <tr>
                  <th className="text-left px-4 py-2 text-gray-600 dark:text-gray-300 font-medium">Producto</th>
                  <th className="text-center px-4 py-2 text-gray-600 dark:text-gray-300 font-medium w-28">Cantidad</th>
                  <th className="text-center px-4 py-2 text-gray-600 dark:text-gray-300 font-medium w-36">Precio unitario</th>
                  <th className="text-right px-4 py-2 text-gray-600 dark:text-gray-300 font-medium w-32">Subtotal</th>
                    ...
                    <tr key={item.producto_id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-4 py-2 text-center">
                        <input type="number" min="0" step="0.01" value={item.precio_unitario}
                          ...
                          className="w-32 border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm text-center focus:outline-none focus:ring-2 focus:ring-blue-500" />
                      </td>
              ...
              <tfoot className="bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-700">
                <tr>
                  ...
                  <td className="px-4 py-3 text-right font-bold text-gray-900 dark:text-gray-100 text-base">
            ...
            <p className="text-center text-gray-400 dark:text-gray-500 text-sm py-6">Buscá productos arriba para agregarlos</p>
          )}
        </div>
        ...
        <div className="flex gap-3">
          <button onClick={() => submit(false)} disabled={loading}
            className="border border-gray-300 dark:border-gray-600 px-5 py-2 rounded text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 transition-colors">
