...
export default function CtaCteClientes() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-xl font-semibold text-gray-800 mb-6">Cuentas corrientes — Clientes</h1>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {data.map((c) => (
              <tr key={c.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/cta-cte/clientes/${c.id}`)}>
                <td className="px-4 py-3 font-medium text-gray-800">{c.nombre}</td>
                <td className="px-4 py-3 text-gray-500">{c.cuit || '—'}</td>
                <td className="px-4 py-3 text-gray-500">{c.telefono || '—'}</td>
                <td className="px-4 py-3 text-right text-gray-600">${fmt(c.limite_credito)}</td>
            ...
            {!data.length && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">Sin clientes con cuenta corriente</td></tr>
            )}
          </tbody>
        </table>
      </div>
