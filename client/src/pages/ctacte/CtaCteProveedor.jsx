...
import { getEstadoCuentaProveedor, pagar, downloadPdfProveedor } from '../../services/ctaCteService';
import api from '../../lib/api';

const fmt = (n) => parseFloat(n || 0).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

export default function CtaCteProveedor() {
  ...
  const [pago, setPagoForm] = useState({ monto: '', descripcion: '', medio_pago_id: '' });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const load = async () => {
    const { data } = await getEstadoCuentaProveedor(proveedorId);
    setEstado(data);
  };
  ...
  useEffect(() => {
    ...
    api.get('/medios-pago').then(({ data }) => {
      setMediosPago(data);
      if (data[0]) setPagoForm((p) => ({ ...p, medio_pago_id: String(data[0].id) }));
    });
  }, [proveedorId]);

  const handlePagar = async () => {
    if (!pago.monto || parseFloat(pago.monto) <= 0) { setError('Ingrese un monto válido'); return; }
    setError('');
    setLoading(true);
    try {
      await pagar({
        ...
        medio_pago_id: pago.medio_pago_id ? parseInt(pago.medio_pago_id) : undefined
      });
      setShowPago(false);
      setPagoForm((p) => ({ ...p, monto: '', descripcion: '' }));
      load();
    } catch (err) {
  ...
  const { proveedor, movimientos, saldo_actual } = estado;
    ...
    <div className="p-6 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-semibold text-gray-800 dark:text-gray-100">{proveedor.nombre}</h1>
          {proveedor.cuit && <p className="text-sm text-gray-500 dark:text-gray-400">CUIT: {proveedor.cuit}</p>}
        </div>
        <div className="text-right">
          <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Deuda actual</p>
          <p className={`text-2xl font-bold ${parseFloat(saldo_actual) > 0 ? 'text-red-600' : 'text-green-700'}`}>
            ${fmt(saldo_actual)}
          </p>
          <p className="text-xs text-gray-400 dark:text-gray-500">{parseFloat(saldo_actual) > 0 ? 'Debemos' : 'Sin deuda'}</p>
        </div>
      </div>
      ...
      {showPago && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-sm space-y-4">
            <h2 className="text-base font-semibold text-gray-800 dark:text-gray-100">Registrar pago a proveedor</h2>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Monto *</label>
              <input type="number" min="0.01" step="0.01" value={pago.monto}
            ...
            <div>
              ...
              <select value={pago.medio_pago_id} onChange={(e) => setPagoForm((p) => ({ ...p, medio_pago_id: e.target.value }))}
                className="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div className="flex gap-3 pt-1">
              <button onClick={handlePagar} disabled={loading}
                className="bg-blue-600 text-white px-5 py-2 rounded text-sm font-medium hover:bg-blue-700 disabled:opacity-50 transition-colors flex-1">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {movimientos.map((m) => (
              <tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td className="px-4 py-2.5 text-gray-700 dark:text-gray-200">{m.descripcion}</td>
