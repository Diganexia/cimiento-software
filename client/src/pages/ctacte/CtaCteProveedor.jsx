    ...
    <div className="p-6 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-semibold text-gray-800">{proveedor.nombre}</h1>
          {proveedor.cuit && <p className="text-sm text-gray-500">CUIT: {proveedor.cuit}</p>}
        </div>
        <div className="text-right">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Deuda actual</p>
          <p className={`text-2xl font-bold ${parseFloat(saldo_actual) > 0 ? 'text-red-600' : 'text-green-700'}`}>
            ${fmt(saldo_actual)}
          </p>
          <p className="text-xs text-gray-400">{parseFloat(saldo_actual) > 0 ? 'Debemos' : 'Sin deuda'}</p>
        </div>
      </div>
      ...
      {showPago && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-sm space-y-4">
            <h2 className="text-base font-semibold text-gray-800">Registrar pago a proveedor</h2>
            <div>
