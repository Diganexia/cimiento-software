  ...
  return (
    <div className="p-6 max-w-lg">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-semibold text-gray-800">Transferencia entre depósitos</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-4">
        <div>
          ...
          {productos.length > 0 && !form.producto_id && (
            <ul className="border border-gray-200 rounded mt-1 divide-y divide-gray-100 max-h-48 overflow-auto">
              {productos.map((p) => (
                <li key={p.id}>
                  <button type="button" onClick={() => { setForm((f) => ({ ...f, producto_id: p.id })); setBusqueda(p.nombre); setProductos([]); }}
                    ...
                    <span>{p.nombre}</span>
                    <span className="text-gray-400 text-xs">Stock: {p.stock_total}</span>
                  </button>
                </li>
              ))}
            </ul>
