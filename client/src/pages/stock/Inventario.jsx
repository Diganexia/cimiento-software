...
export default function Inventario() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-5">Inventario físico</h1>
        ...
        {['Abrir', 'Contar', 'Revisar', 'Confirmar'].map((label, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2">
              ...
              <div className={`flex items-center gap-1.5 ${active ? 'text-blue-600 font-medium' : done ? 'text-gray-500 dark:text-gray-400' : 'text-gray-400 dark:text-gray-500'}`}>
                <span className={`w-5 h-5 rounded-full text-xs flex items-center justify-center ${active ? 'bg-blue-600 text-white' : done ? 'bg-gray-200 text-gray-600 dark:text-gray-300' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500'}`}>{i + 1}</span>
                {label}
              </div>
            </div>
          );
      ...
      {/* Step 1: Abrir */}
      {step === STEP.INICIO && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 max-w-sm space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-300">Seleccioná el depósito a inventariar. Se va a bloquear para edición durante el proceso.</p>
          <div>
      ...
      {step === STEP.CONTEO && inventario && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
            <div>
              <p className="font-medium text-gray-800 dark:text-gray-100">Depósito: {inventario.deposito}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">{inventario.items.length} productos — ingresá la cantidad contada de cada uno</p>
            </div>
          ...
          <div className="overflow-x-auto max-h-[60vh] overflow-y-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-700 sticky top-0">
                <tr>
                  <th className="text-left px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">Producto</th>
                  <th className="text-right px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">Stock sistema</th>
                  <th className="text-center px-4 py-3 text-gray-600 dark:text-gray-300 font-medium w-36">Cantidad contada</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {inventario.items.map((item) => (
                  <tr key={item.producto_id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td className="px-4 py-2">
                      <p className="font-medium text-gray-800 dark:text-gray-100">{item.producto}</p>
                      {item.codigo && <p className="text-xs text-gray-400 dark:text-gray-500 font-mono">{item.codigo}</p>}
                    </td>
                    <td className="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{parseFloat(item.cantidad_sistema)} {item.unidad}</td>
                    <td className="px-4 py-2 text-center">
                      <input
                        ...
                        className="w-28 border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm text-center focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </td>
                  </tr>
                ))}
      ...
      {step === STEP.DIFERENCIAS && inventario && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
            <div>
              <p className="font-medium text-gray-800 dark:text-gray-100">Revisión de diferencias — {inventario.deposito}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">{hasDiferencias ? 'Hay diferencias que serán ajustadas automáticamente al confirmar.' : 'No hay diferencias.'}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={handleCancelar} disabled={loading}
                className="border border-red-300 text-red-600 px-3 py-2 rounded text-sm hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-700">
                <tr>
                  <th className="text-left px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">Producto</th>
                  <th className="text-right px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">Sistema</th>
                  <th className="text-right px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">Contado</th>
                  <th className="text-right px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">Diferencia</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {inventario.items.filter((i) => i.cantidad_contada !== null).map((item) => {
                  ...
                  return (
                    <tr key={item.producto_id} className={`hover:bg-gray-50 dark:hover:bg-gray-700 ${dif !== 0 ? 'bg-yellow-50 dark:bg-yellow-900/20' : ''}`}>
                      ...
                      <td className={`px-4 py-2 text-right font-semibold ${dif > 0 ? 'text-green-600' : dif < 0 ? 'text-red-600' : 'text-gray-400 dark:text-gray-500'}`}>
                        {dif > 0 ? '+' : ''}{dif}
                      </td>
                    </tr>
                  );
      ...
      {step === STEP.CONFIRMADO && (
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6 text-center max-w-sm">
          ...
          <button onClick={() => { setStep(STEP.INICIO); setInventario(null); setInventarioId(null); setDepositoId(''); setConteos({}); }}
            className="bg-white dark:bg-gray-800 border border-green-300 text-green-700 px-4 py-2 rounded text-sm hover:bg-green-50 transition-colors">
            Nuevo inventario
          </button>
        </div>
      )}
