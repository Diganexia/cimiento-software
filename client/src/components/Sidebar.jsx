...
function canSee(permisos, perm) {
  ...
  return permisos[mod]?.[action] === true;
}
...
const NAV = [
  ...
  { type: 'section', label: 'Facturación' },
  {
    label: 'Facturación',
    to: '/facturacion',
    perm: 'facturacion.ver',
    icon: (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  { type: 'section', label: 'Tesorería' },
  {
    label: 'Caja',
    to: '/tesoreria/caja',
    perm: 'caja.ver_movimientos',
    icon: (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  { type: 'section', label: 'Cta. Corriente' },
  {
    label: 'Clientes',
    to: '/cta-cte/clientes',
    perm: 'cta_cte.ver',
      ...
      <nav className="flex-1 py-3 px-2 space-y-0.5 overflow-y-auto">
        {NAV.map((item, i) => {
          if (item.type === 'section') {
            ...
            const items = nextSection === -1 ? rest : rest.slice(0, nextSection);
            if (!items.some(x => canSee(usuario?.permisos, x.perm))) return null;
            return (
              <p key={i} className="px-3 pt-3 pb-1 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                {item.label}
              </p>
            );
          }
          if (!canSee(usuario?.permisos, item.perm)) return null;
          return (
            <NavLink
              key={item.to}
              to={item.to}
