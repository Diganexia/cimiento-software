...
function generarVentaPDF(venta, items, cliente, comprobante, res) {
  ...
  // Tipo + número
  doc.fontSize(25).font('Helvetica-Bold').fillColor('#000')
  ...
  if (cliente) {
    doc.text(cliente.nombre || cliente.razon_social || 'Sin nombre', 50, y + 15);
  ...
  if (comprobante?.cae) {
    ...
    doc.fontSize(10).text(`Código de barras: ${barcode}`, 50, y, { width: 495 });
  }

  // Footer anclado al fondo de la página actual
  const footerYVenta = doc.page.height - doc.page.margins.bottom - 14;
  doc.fontSize(10).fillColor('#999')
    .text('Documento no válido como factura', 50, footerYVenta, { align: 'center', width: 495 });
...
function generarEstadoCuentaPDF({ entidad, movimientos, saldo, tipo }, res) {
  ...
  // Header empresa
  doc.fontSize(17).font('Helvetica-Bold').text(emp.nombre, 50, 50);
  ...
  doc.fontSize(17).font('Helvetica-Bold').fillColor('#000')
    .text(`Estado de Cuenta — ${esCliente ? 'Cliente' : 'Proveedor'}`, 50, 105);
  doc.fontSize(13).font('Helvetica').fillColor('#333')
    .text(entidad.nombre || entidad.razon_social, 50, 124);
  if (entidad.cuit) doc.text(`CUIT: ${entidad.cuit}`, 50, 138);

  const ahora = new Date();
  const fechaImpresion = ahora.toLocaleDateString('es-AR');
  const horaImpresion = ahora.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' });
  doc.fontSize(11).text(`Impreso: ${fechaImpresion} ${horaImpresion}`, 370, 105, { align: 'right', width: 175 });
  ...
  doc.fontSize(10).fillColor('#999')
    .text(parseFloat(saldo) > 0 ? 'Saldo deudor' : 'Saldo acreedor / sin deuda', 50, footerYCtaCte, { align: 'center', width: 495 });
...
function generarArqueoPDF({ arqueo, movimientos, resumen, ingresos, egresos, saldo_calculado }, res) {
  ...
  doc.fontSize(17).font('Helvetica-Bold').fillColor('#000').text('RESUMEN DE CAJA', 350, 50, { align: 'right', width: 195 });
  doc.fontSize(12).font('Helvetica').fillColor('#333')
    .text(`Caja: ${arqueo.caja}`, 350, 74, { align: 'right', width: 195 })
    .text(`Operador: ${arqueo.usuario_apertura}`, 350, 88, { align: 'right', width: 195 });

  const fmtDate = (d) => d ? new Date(d).toLocaleString('es-AR') : '—';
  doc.text(`Apertura: ${fmtDate(arqueo.abierto_at)}`, 350, 102, { align: 'right', width: 195 });
  if (arqueo.cerrado_at) doc.text(`Cierre: ${fmtDate(arqueo.cerrado_at)}`, 350, 116, { align: 'right', width: 195 });

  doc.moveTo(50, 138).lineTo(545, 138).strokeColor('#ccc').stroke();

  // Saldo inicial
  let y = 153;
  doc.fontSize(12).font('Helvetica').fillColor('#555').text('Saldo inicial:', 50, y);
  ...
  y += 17;
  doc.font('Helvetica-Bold').fontSize(14).fillColor('#000').text('Saldo calculado:', 50, y);
  ...
  y += 13;
  doc.font('Helvetica-Bold').fontSize(11).fillColor('#555').text('DETALLE DE MOVIMIENTOS', 50, y);
  y += 17;
  ...
  doc.font('Helvetica').fillColor('#000').fontSize(10.5);
...
function generarReporteTablaPDF({ titulo, subtitulo, columnas, filas, totales, landscape = false }, res) {
  ...
  // Header
  doc.fontSize(13).font('Helvetica-Bold').fillColor('#000').text(emp.nombre, 40, 30);
  doc.fontSize(11).font('Helvetica').fillColor('#777')
    .text(`Generado: ${new Date().toLocaleString('es-AR')}`, 40, 47);

  doc.fontSize(17).font('Helvetica-Bold').fillColor('#000').text(titulo, 40, 65);
  let headerBottom = 85;
  if (subtitulo) {
    doc.fontSize(11).font('Helvetica').fillColor('#555').text(subtitulo, 40, 85);
    headerBottom = 100;
  }
  ...
  // Column headers
  let x = 40;
  doc.fontSize(10.5).font('Helvetica-Bold').fillColor('#555');
  ...
  if (!filas.length) {
    doc.font('Helvetica').fontSize(12).fillColor('#999').text('Sin datos para mostrar', 40, y + 10, { align: 'center', width: pageW });
  }
...
function generarReciboPDF({ cobro, cliente, medioPago, retenciones = [] }, res) {
  ...
  doc.fontSize(14).font('Helvetica')
    .text(`N° ${String(cobro.id).padStart(8, '0')}`, 350, 84, { align: 'right', width: 200 });
  const fecha = cobro.created_at
    ? new Date(cobro.created_at).toLocaleDateString('es-AR')
    : new Date().toLocaleDateString('es-AR');
  doc.text(`Fecha: ${fecha}`, 350, 101, { align: 'right', width: 200 });

  doc.moveTo(50, 140).lineTo(545, 140).strokeColor('#ccc').stroke();

  // Cliente
  let y = 155;
  doc.fontSize(12).font('Helvetica-Bold').fillColor('#000').text('CLIENTE', 50, y);
  doc.font('Helvetica').fillColor('#333');
  doc.text(cliente.nombre || cliente.razon_social || '', 50, y + 15);
  if (cliente.cuit) doc.text(`CUIT: ${cliente.cuit}`, 50, y + 30);
  else if (cliente.dni) doc.text(`DNI: ${cliente.dni}`, 50, y + 30);
  if (cliente.direccion) doc.text(cliente.direccion, 50, y + 45);

  // Medio de pago
  if (medioPago) {
    doc.font('Helvetica-Bold').fillColor('#000').text('Medio de pago:', 350, y + 15, { width: 200, align: 'right' });
    doc.font('Helvetica').fillColor('#333').text(medioPago.nombre, 350, y + 30, { width: 200, align: 'right' });
    if (medioPago.numero_cheque) {
      doc.text(`Cheque N° ${medioPago.numero_cheque}`, 350, y + 45, { width: 200, align: 'right' });
      if (medioPago.banco) doc.text(`Banco: ${medioPago.banco}`, 350, y + 60, { width: 200, align: 'right' });
    }
  }
  ...
  // Detalle cobro
  doc.fontSize(12).font('Helvetica-Bold').fillColor('#555').text('Concepto', 50, y, { width: 310 });
  doc.text('Importe', 365, y, { width: 180, align: 'right' });
  y += 15;
  doc.moveTo(50, y).lineTo(545, y).strokeColor('#ccc').stroke();
  y += 10;

  doc.font('Helvetica').fillColor('#000').fontSize(13);
  ...
  if (retenciones.length > 0) {
    ...
    y += 10;
    doc.font('Helvetica').fontSize(12).fillColor('#555').text('Total retenciones:', 50, y, { width: 310 });
  ...
  doc.font('Helvetica-Bold').fontSize(15).fillColor('#000').text('NETO COBRADO:', 50, y);
  doc.fontSize(16).text(`$${fmt(netoCobrado)}`, 365, y, { width: 180, align: 'right' });
  y += 30;
  ...
  y += 8;
  doc.fontSize(11).fillColor('#333').text('Firma y aclaración cliente', 50, y, { width: 160, align: 'center' });
  doc.text('Firma y sello empresa', 335, y, { width: 210, align: 'center' });

  const footerYRecibo = doc.page.height - doc.page.margins.bottom - 14;
  doc.fontSize(10).fillColor('#999')
    .text('Recibo válido como comprobante de pago', 50, footerYRecibo, { align: 'center', width: 495 });
